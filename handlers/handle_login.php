...
if ($result->num_rows > 0) {
    ...
    if ($password == $row['password']) {
        ...
        header("Location: ../pages/MainStyleWithout.php?welcome=1"); // Redirect to MainPage.php with welcome parameter
        exit();
    } else {
        // Password is incorrect, redirect back to login page with error message
        header("Location: ../pages/LoginPage.php?error=incorrect_password"); // Redirect with error parameter
        exit();
    }
} else {
    // Email not found, redirect back to login page with error message
    header("Location: ../pages/LoginPage.php?error=email_not_found"); // Redirect with error parameter
    exit();
}
